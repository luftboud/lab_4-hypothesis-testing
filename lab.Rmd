...
$H_0 : µ_1 = µ_2 \quad vs. \quad H_1 : µ_1  \neq µ_2;\quad σ^2_1 = σ^2_2 = 1.$

Here we should use z-test, because $\sigma^2$ is known:

$$
Z=\frac{​​\bar X−\bar Y}{\sqrt{\sigma^2_1​/n_1​+\sigma_2^2​/n_2}​}=\frac{​​\bar X−\bar Y}{\sqrt{1/100+1/50}​}​​=\frac{​​\bar X−\bar Y}{\sqrt{0.03}​}​​.
$$
...
# Test statistic:
z_stat <- (mean(xk) - mean(yl)) / sqrt(sigma1^2 / n1 + sigma2^2 / n2)
...
cat("Rejection region at level", alpha, ": |Z| >", z_crit, "\n\n")

cat("p-value =", p_val, "\n")

if (abs(z_stat) > z_crit) {
  cat("Decision at alpha =", alpha, ": reject H0.\n")
} else {
  cat("Decision at alpha =", alpha, ": do NOT reject H0.\n")
}

```

For a two-sided test at level 0.05 the rejection region is $|Z| > z_{0.975} \approx 1.96$. In our data the observed z-statistic is very close to 0 and the corresponding p-value is about $p \approx 0.97$, which is much larger than $0.05$. Therefore we do **not** reject $H_0$​. The sample does not provide evidence that the two means are different.
